  ...
  return (
    <header
      className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${
        isScrolled 
          ? 'bg-white/90 backdrop-blur-md border-b border-purple-100 shadow-sm py-3' 
          : 'bg-transparent py-5'
      }`}
    >
      <div className="container px-4 md:px-6 flex items-center justify-between">
        ...
        <div className="hidden md:flex items-center gap-3">
          <Link to="/login">
            <Button variant="outline" size="sm" className="gap-2 border-purple-200 hover:bg-purple-50 hover:text-purple-700 transition-all duration-300">
          ...
          <Toggle 
            ...
            variant="outline"
            className={`rounded-full ${notificationsEnabled ? 'bg-black text-white' : ''}`}
          >
          ...
          <div className="relative">
            ...
            {isUserMenuOpen && (
              <ClickOutside onClickOutside={() => setIsUserMenuOpen(false)} className="absolute right-0 mt-2 w-56 z-50">
                <div className="bg-white rounded-md shadow-md border py-1">
                  <div className="px-4 py-2 text-sm font-medium border-b">My Account</div>
                  <Link to="/settings" className="block px-4 py-2 text-sm hover:bg-purple-50" onClick={() => setIsUserMenuOpen(false)}>
                    <div className="flex items-center">
                      <Settings className="mr-2 h-4 w-4" />
                      <span>Settings</span>
                    </div>
                  </Link>
                  <Link to="/" className="block px-4 py-2 text-sm hover:bg-purple-50" onClick={() => setIsUserMenuOpen(false)}>
                    <div className="flex items-center">
      ...
      {/* Mobile Menu */}
      {isMobileMenuOpen && (
        <ClickOutside onClickOutside={() => setIsMobileMenuOpen(false)} className="fixed inset-0 z-40 bg-white pt-20 animate-in">
          <div ref={mobileMenuRef} className="container px-4 flex flex-col gap-4 py-8">
            <button 
              onClick={() => scrollToSection('home')}
              className="text-lg font-medium py-3 px-4 hover:bg-purple-50 rounded-md transition-colors duration-300"
            >
              Home
            </button>
            <button 
            ...
            <div className="mt-4 space-y-2">
              ...
              >
                <Button variant="outline" className="w-full justify-start gap-2 border-purple-200">
                  <LogIn className="h-4 w-4" />
                  Sign In
                </Button>
              </Link>
